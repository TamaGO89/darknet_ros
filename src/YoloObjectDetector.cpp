...
// Check for xServer
#include <X11/Xlib.h>  // TODO : I don't need an X-server!
...
void YoloObjectDetector::init() {
  ...
  objectPublisher_ =
      nodeHandle_.advertise<darknet_ros::ObjectCount>(objectDetectorTopicName, objectDetectorQueueSize, objectDetectorLatch);
  boundingBoxesPublisher_ =
      nodeHandle_.advertise<darknet_ros::BoundingBoxes>(boundingBoxesTopicName, boundingBoxesQueueSize, boundingBoxesLatch);
...
void YoloObjectDetector::checkForObjectsActionGoalCB() {
  ...
  boost::shared_ptr<const darknet_ros::CheckForObjectsGoal> imageActionPtr = checkForObjectsActionServer_->acceptNewGoal();
...
detection* YoloObjectDetector::avgPredictions(network* net, int* nboxes) {
  ...
  // EDIT : The new definition of get_network_boxes wants another argument (letter)
...
void* YoloObjectDetector::detectInThread() {
  ...
  float* prediction = network_predict(*net_, X);  // DONE : net_ became a pointer... Check this
  ...
  draw_detections_v3(display, dets, nboxes, demoThresh_, demoNames_, demoAlphabet_, demoClasses_, 0);  // EDIT : last args is used as a bool but it's defined as an int
...
void* YoloObjectDetector::fetchInThread() {
  {
    ...
    IplImageWithHeader_ imageAndHeader = getIplImageWithHeader();  // EDIT : Rename this
    // IplImage* ROS_img = imageAndHeader.image;  // DONE : Remove dependency
    cv::Mat ROS_img = *imageAndHeader.image;  // DONE : This is a cv::Mat now  // DONE : This became a pointer
    // ipl_into_image(ROS_img, buff_[buffIndex_]);  // EDIT : ipl_into_image should now move cv::Mat to image
    buff_[buffIndex_] = mat_to_image(ROS_img);  // EDIT : ipl_into_image should now move cv::Mat to image
...
void* YoloObjectDetector::displayInThread(void* ptr) {
  /*
  SOURCE CODE : This simply shows an image, it has a return value but here isn't used
  int show_image_cv(image im, const char* name, int ms) {
      Mat m = image_to_mat(im); imshow(name, m); int c = waitKey(ms); if (c != -1) c = c%256; return c; }
  */
  show_image_cv(buff_[(buffIndex_ + 1) % 3], "YOLO V3"); //, ipl_);  // DONE : I can't find a replacement for this function!!
...
void YoloObjectDetector::yolo() {
...

  {
    ...
    // IplImage* ROS_img = imageAndHeader.image;  // DONE : Remove dependency
    cv::Mat ROS_img = *imageAndHeader.image;  // DONE : This is a cv::Mat now  // DONE : This became a pointer
    // buff_[0] = ipl_to_image(ROS_img);  // DONE : Rename this (mat_to_image)
    buff_[0] = mat_to_image(ROS_img);  // DONE : Rename this (mat_to_image)
  ...
  // ipl_ = cvCreateImage(cvSize(buff_[0].w, buff_[0].h), IPL_DEPTH_8U, buff_[0].c);  // DONE : cvCreateImage now creates cv::Mat
  ipl_ = new cv::Mat(buff_[0].w, buff_[0].h, CV_8UC(buff_[0].c)); // cvCreateImage(cvSize(416, 416), IPL_DEPTH_8U, 3);
  ...
  while (!demoDone_) {
    ...
    if (!demoPrefix_) {
      ...
      if (viewImage_) {
        ...
        // generate_image(buff_[(buffIndex_ + 1) % 3], ipl_);  // DONE : Remove dependency, generate_image now works with image and cv::Mat
        *ipl_ = image_to_mat(buff_[(buffIndex_ + 1) % 3]);  // EDIT : Potential alternative solution
      }
      publishInThread();
    } else {
      char name[256];
...
/*
IplImageWithHeader_ YoloObjectDetector::getIplImageWithHeader() {  // DONE : Remove dependency
  IplImage* ROS_img = new IplImage(camImageCopy_);  // DONE : Remove dependency
  IplImageWithHeader_ header = {.image = ROS_img, .header = imageHeader_};  // DONE : Remove dependency
  return header;
}
*/
// DONE : Replace IplImage with cv::Mat (new IplImage takes a cv::Mat object)
IplImageWithHeader_ YoloObjectDetector::getIplImageWithHeader() {
  IplImageWithHeader_ header = {.image = &camImageCopy_, .header = imageHeader_};  // TODO : camImageCopy_ became &camImageCopy_ ... I've no idea what i'm doing
  return header;
}
...
void* YoloObjectDetector::publishInThread() {
  // Publish image.
  /*
  cv::Mat cvImage = cv::cvarrToMat(ipl_);  // DONE : ipl_ will already be a cv::Mat
  if (!publishDetectionImage(cv::Mat(cvImage))) {
    ROS_DEBUG("Detection image has not been broadcasted.");
  }
  */
  if (!publishDetectionImage(cv::Mat(*ipl_))) {  // DONE : ipl_ is already a cv::Mat
    ROS_DEBUG("Detection image has not been broadcasted.");
  }
  ...
  if (num > 0 && num <= 100) {
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        darknet_ros::BoundingBox boundingBox;
  ...
  if (isCheckingForObjects()) {
    ...
    darknet_ros::CheckForObjectsResult objectsActionResult;
